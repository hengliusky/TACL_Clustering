...
def sample_graph_threshold(adj_orig, A_pred, remove_threshold, add_threshold):
    # Process edge removal
    adj_upper = sp.triu(adj_orig, 1).tocsr()  # 确保转换为CSR格式
    edges = np.column_stack(adj_upper.nonzero())
    if edges.size > 0:
        probs = A_pred[edges[:, 0], edges[:, 1]]
        mask_remove = probs >= remove_threshold
        edges_remain = edges[mask_remove]
    else:
        edges_remain = edges

    adj_remove = sp.csr_matrix((np.ones(len(edges_remain)), edges_remain.T),
                               shape=adj_orig.shape)
    adj_remove = adj_remove.maximum(adj_remove.T)  # 更高效的对称化方式

    # Process edge addition
    n = adj_orig.shape[0]
    existing_upper = sp.triu(adj_orig, 1).tocsr()  # 转换为CSR格式
    rows, cols = np.triu_indices(n, 1)

    # 使用高效的方式检查边存在性
    existing_mask = np.asarray(existing_upper[rows, cols]).flatten().astype(bool)

    candidate_probs = A_pred[rows, cols]
    mask_add = (~existing_mask) & (candidate_probs >= add_threshold)
    new_edges = np.column_stack((rows[mask_add], cols[mask_add]))

    # 使用lil_matrix进行高效的元素修改
    adj_add = adj_orig.copy().tolil()
    adj_add[new_edges[:, 0], new_edges[:, 1]] = 1
    adj_add[new_edges[:, 1], new_edges[:, 0]] = 1

    return adj_remove.tocsr(), adj_add.tocsr()
...
def cos_sim(feat_a, feat_b):
    return np.dot(feat_a, feat_b) / (np.linalg.norm(feat_a) * np.linalg.norm(feat_b))
...
def load_data(dataset):
    ...
    for i in range(len(names)):
        ...
        '''
        with open("data/{}/ind.{}.{}".format(dataset, dataset, names[i]), 'rb') as rf:
            u = pkl._Unpickler(rf)
            u.encoding = 'latin1'
            cur_data = u.load()
    ...
    if dataset == 'acm':
        test_idx_reorder = list(range(640, 3025))
...
def my_preprocess_graph(adj, norm='sym', renorm=True):
    adj = sp.coo_matrix(adj)
    ident = sp.eye(adj.shape[0])
    if renorm:
        adj_ = adj + ident
